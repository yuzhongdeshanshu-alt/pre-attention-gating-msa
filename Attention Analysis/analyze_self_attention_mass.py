#!/usr/bin/env python3
"""Compute self-attention macro allocation on the test split.

The exported per-sample table contains the final-layer mass metrics used by the
self-attention allocation figure:

    row_weighted_intra
    row_weighted_inter

The script loads the NoGate and PreGate self-attention checkpoints, runs the
test split once, and writes one per-sample row for the selected layer.
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Export self-attention intra/inter mass for the final figure.")
    ...
    parser.add_argument("--plot-layer", type=int, default=None, help=argparse.SUPPRESS)
    parser.add_argument(
        "--mask-modes",
        nargs="+",
    ...
    parser.add_argument("--case-mask-mode", type=str, default="include_special", help=argparse.SUPPRESS)
...
def compute_macro_mass(attn_mean: torch.Tensor, modality_masks: List[torch.Tensor]) -> List[Dict[str, float]]:
    ...
    for b_idx in range(attn_mean.shape[0]):
        ...
        # key modality, averaged over valid query rows in that modality.
        block = torch.full((3, 3), float("nan"), dtype=attn_mean.dtype, device=attn_mean.device)
        query_counts = torch.zeros(3, dtype=attn_mean.dtype, device=attn_mean.device)
        for q_idx in range(3):
            q_mask = modality_masks[q_idx][b_idx]
            q_count = q_mask.sum()
            query_counts[q_idx] = q_count
            if int(q_count.item()) == 0:
                continue
            for k_idx in range(3):
                k_mask = modality_masks[k_idx][b_idx]
                if int(k_mask.sum().item()) == 0:
                    continue
        ...
        # Weight modality-level rows by their number of valid query positions so
        # each token or frame contributes equally to the sample-level estimate.
        total_queries = query_counts.sum().clamp_min(1)
        rows.append(
            {
                "batch_index": float(b_idx),
                "row_weighted_intra": safe_float(torch.nansum(intra_by_query * query_counts) / total_queries),
                "row_weighted_inter": safe_float(torch.nansum(inter_by_query * query_counts) / total_queries),
            }
        )
    return rows
...
def analyze_condition(
    ...
    for step, batch_cpu in enumerate(loader, start=1):
        ...
        for mask_mode in mask_modes:
            ...
            for metric_row in compute_macro_mass(attn_mean, modality_masks):
...
def summarize(rows: Sequence[Dict[str, object]]) -> List[Dict[str, object]]:
    ...
    for condition, condition_label, mask_mode, layer in groups:
        ...
        for metric in ["row_weighted_intra", "row_weighted_inter"]:
            n, mean_value, sd_value = finite_mean_sd(r[metric] for r in selected)
            out.append(
                {
                    "condition": condition,
...
def selected_layer(args: argparse.Namespace, num_layers: int) -> int:
    requested = args.plot_layer if args.plot_layer is not None else args.layer
    layer = num_layers - 1 if requested < 0 else requested
    if layer < 0 or layer >= num_layers:
        raise ValueError(f"Layer must be in [0, {num_layers - 1}] or -1, got {requested}")
    return layer
...
def main() -> None:
    ...
    per_sample_path = args.output_dir / "attention_allocation_per_sample.csv"
    summary_path = args.output_dir / "attention_block_mass_summary.csv"
    write_csv(all_rows, per_sample_path)
    write_csv(summarize(all_rows), summary_path)

    metadata = {
        "metric_purpose": "self-attention macro allocation",
    ...
    with (args.output_dir / "attention_allocation_metadata.json").open("w", encoding="utf-8") as handle:
        json.dump(metadata, handle, indent=2)
    print(f"Saved self-attention allocation outputs to: {args.output_dir}")
