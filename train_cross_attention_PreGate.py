"""Cross-attention model with pre-softmax gating for CMU-MOSEI."""
...
MODEL_DEFAULTS = {
    "run_name": "cross_attention_pregate_seed10",
    "base_lr": 3e-5,
    "weight_decay": 0.0,
    "batch_train": 48,
    "batch_eval": 48,
...
class CrossAttentionLayer(nn.Module):
    """One cross-attention layer with pre-softmax gating."""
    ...
    @staticmethod
    def _bounded_prob(x: torch.Tensor) -> torch.Tensor:
        """Map projected features to a bounded positive gate value."""
        # Clamping avoids exact zeros, which would erase a query/source row
        # before the subsequent softmax normalization.
        return torch.sigmoid(x).clamp(min=PRE_GATE_EPS, max=1.0)

    def _compute_pre_pair_prob(self, q: torch.Tensor, k: torch.Tensor) -> torch.Tensor:
        """Build a pairwise pre-softmax scaling matrix from query/key projections."""
        # Average feature-wise gate activations into one scalar per query token
        # and source token, then form their pairwise interaction.
        q_prob = self._bounded_prob(q).mean(dim=-1, keepdim=True)
        k_prob = self._bounded_prob(k).mean(dim=-1, keepdim=True).transpose(1, 2)
        return (q_prob * k_prob).clamp(min=PRE_GATE_EPS, max=1.0)
    ...
    def _cross_attention(
        ...
        """Project one query/source modality pair and return gated attended features."""
        ...
        # The pre-gate changes the attention distribution before masking and
        # softmax; the value vectors themselves are left unchanged.
        scores = scores * self._compute_pre_pair_prob(q, k)
    ...
    def forward(
        ...
        """Update each modality using gated cross-attention from the other modalities."""
...
def main() -> None:
    """Train, evaluate, and save a pre-gating cross-attention model."""
    ...
    result_payload = {
        ...
        "config_name": "cross_attention_pregate",
